...
__inline void calculate_distance(int b) {
    for (j = 0; j < M; j += 4) {
        ...
        if (j + 1 < M)
            buffer2[i + b][j + 1] = f[!buffer1[b][j + 1]];
        if (j + 2 < M)
            buffer2[i + b][j + 2] = f[!buffer1[b][j + 2]];
        if (j + 3 < M)
            buffer2[i + b][j + 3] = f[!buffer1[b][j + 3]];
    }
}
...
__inline void check_neighbors(int b) {
    for (j = 0; j < M; j += 2) {
        if (buffer2[i + b][j] == k - 1) {
            if (i + b > 0 && buffer2[i + b - 1][j] == -1)
                buffer2[i + b - 1][j] = k;
            if (i + b < (N - 1) && buffer2[i + b + 1][j] == -1)
                buffer2[i + b + 1][j] = k;
            if (j > 0 && buffer2[i + b][j - 1] == -1)
                buffer2[i + b][j - 1] = k;
            if (j < (M - 1) && buffer2[i + b][j + 1] == -1)
                buffer2[i + b][j + 1] = k;
        }
        if (j + 1 < M) {
            if (buffer2[i + b][j + 1] == k - 1) {
                if (i + b > 0 && buffer2[i + b - 1][j + 1] == -1)
                    buffer2[i + b - 1][j + 1] = k;
                if (i + b < (N - 1) && buffer2[i + b + 1][j + 1] == -1)
                    buffer2[i + b + 1][j + 1] = k;
                if (j > -1 && buffer2[i + b][j] == -1)
                    buffer2[i + b][j] = k;
                if (j < (M - 2) && buffer2[i + b][j + 2] == -1)
                    buffer2[i + b][j + 2] = k;
            }
        }
    }
