...
void distance_transformer() {
    ...
    for (i = 0; i < N; i++) {
        for (j = 0; j < M; j += 4) {
            ...
            if (j + 1 < M) distance[i][j + 1] = (input[i][j + 1] == 0) ? 0 : -1;
            if (j + 2 < M) distance[i][j + 2] = (input[i][j + 2] == 0) ? 0 : -1;
            if (j + 3 < M) distance[i][j + 3] = (input[i][j + 3] == 0) ? 0 : -1;
        }
    }
    ...
    for (i = 0; i < N; i++) {
        for (j = 0; j < M; j += 4) {
            ...
            if (j + 1 < M) current_y[i][j + 1] = distance[i][j + 1];
            if (j + 2 < M) current_y[i][j + 2] = distance[i][j + 2];
            if (j + 3 < M) current_y[i][j + 3] = distance[i][j + 3];
        }
    }
}
